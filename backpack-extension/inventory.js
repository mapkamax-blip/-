...
async function dropItem(entryId) {
  ...
  const dropped = buildImage(entry.image, entry.grid ?? { dpi: 150, offset: { x: 75, y: 75 } }).name(entry.name).scale(entry.scale ?? { x: 1, y: 1 }).position({
    x: character.position.x + jitter(),
    y: character.position.y + jitter()
  }).layer(entry.layer || "PROP").build();
