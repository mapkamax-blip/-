...
async function init() {
  ...
  unsubscribe = lib_default.scene.items.onChange(handleItemsChange);
...
function renderList(list) {
  ...
  for (const entry of list) {
    ...
    const plainText = entry.text?.plainText;
    if (plainText) {
      name.textContent = `${entry.name} \u2014 ${plainText}`;
    }
...
async function dropItem(entryId) {
  ...
  const builder = buildImage(entry.image, entry.grid ?? { dpi: 150, offset: { x: 75, y: 75 } }).name(entry.name).scale(entry.scale ?? { x: 1, y: 1 }).position({
  ...
  if (entry.textItemType) {
    builder.textItemType(entry.textItemType);
  }
  const dropped = builder.build();
  await lib_default.scene.items.addItems([dropped]);
  lib_default.notification.show(`\xAB${entry.name}\xBB \u043B\u0435\u0436\u0438\u0442 \u043D\u0430 \u0441\u0442\u043E\u043B\u0435`, "DEFAULT");
}
