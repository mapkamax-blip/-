...
// src/edit.entry.js
var ID = "rodeo.maxim.charsheet";
var DATA_KEY = `${ID}/data`;
var characterId = null;
var app = document.querySelector("#app");
lib_default.onReady(init);
async function init() {
  const selection = await lib_default.player.getSelection();
  if (!selection || !selection.length) {
    app.innerHTML = `<p class="empty">\u0412\u044B\u0434\u0435\u043B\u0438\u0442\u0435 \u0442\u043E\u043A\u0435\u043D \u043F\u0435\u0440\u0441\u043E\u043D\u0430\u0436\u0430</p>`;
    return;
  }
  characterId = selection[0];
  const items = await lib_default.scene.items.getItems([characterId]);
  const token = items[0];
  const card = token?.metadata[DATA_KEY] || {
    name: token?.name || "",
    race: "",
    stats: {},
    traits: []
...
function render(card) {
  ...
  app.innerHTML = `
    <label>\u0418\u043C\u044F
      <input id="f-name" type="text" value="${attr(card.name)}" />
    </label>
    <label>\u0420\u0430\u0441\u0430
      <input id="f-race" type="text" value="${attr(card.race)}" />
    </label>
    <div class="stats-grid">
      <label>\u0421<input id="f-s" type="number" value="${attr(stats.s ?? 0)}" /></label>
      <label>\u041B<input id="f-l" type="number" value="${attr(stats.l ?? 0)}" /></label>
      <label>\u0423<input id="f-u" type="number" value="${attr(stats.u ?? 0)}" /></label>
      <label>\u0416<input id="f-zh" type="number" value="${attr(stats.zh ?? 0)}" /></label>
    </div>
    <label>\u0421\u043F\u043E\u0441\u043E\u0431\u043D\u043E\u0441\u0442\u0438 (\u043A\u0430\u0436\u0434\u0430\u044F \u0441 \u043D\u043E\u0432\u043E\u0439 \u0441\u0442\u0440\u043E\u043A\u0438)
      <textarea id="f-traits" rows="5">${escapeHtml((card.traits || []).join("\n"))}</textarea>
    </label>
...
async function save() {
  const data = {
    ...
    race: document.querySelector("#f-race").value.trim(),
    stats: {
      s: Number(document.querySelector("#f-s").value) || 0,
      l: Number(document.querySelector("#f-l").value) || 0,
      u: Number(document.querySelector("#f-u").value) || 0,
      zh: Number(document.querySelector("#f-zh").value) || 0
    },
    traits: document.querySelector("#f-traits").value.split("\n").map((s) => s.trim()).filter(Boolean)
  };
  const items = await lib_default.scene.items.getItems([characterId]);
  await lib_default.scene.items.updateItems(items, (drafts) => {
    for (const draft of drafts) {
      draft.metadata[DATA_KEY] = data;
    }
  });
  lib_default.notification.show("\u041A\u0430\u0440\u0442\u043E\u0447\u043A\u0430 \u0441\u043E\u0445\u0440\u0430\u043D\u0435\u043D\u0430", "SUCCESS");
}
function attr(v) {
